...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (currentTime - messageTime > 300) {
        reject(new Error("Time exceed: More than five minutes"));
      }
  ...
  getBlockByHash(hash) {
    ...
    return new Promise((resolve, reject) => {
      const foundBlock = self.chain.filter(
        (currentBlock) => currentBlock.hash === hash
      );
      if (foundBlock.length === 0) {
        reject(new Error("A block with the passed hash does not exist"));
      }
  ...
  /**
   * Private method that returns current timestamp
   */
